...
#define EXAMPLE_TX_DMA_SOURCE \
  (FLEXIO_DMA_REQUEST_SOURCE_BASE + FLEXIO_TX_SHIFTER_INDEX)
#define EXAMPLE_RX_DMA_SOURCE \
  (FLEXIO_DMA_REQUEST_SOURCE_BASE + FLEXIO_RX_SHIFTER_INDEX)
...
void LPSPI_SlaveUserCallback(LPSPI_Type *base, lpspi_slave_handle_t *handle,
                             status_t status, void *userData);
...
int main(void) {
  ...
  PRINTF(
      "This example use one flexio spi as master and one lpspi instance as "
      "slave on one board.\r\n");
  PRINTF("Master uses edma and slave uses interrupt way.\r\n");
  PRINTF(
      "Please make sure you make the correct line connection. Basically, the "
      "connection is:\r\n");
  ...
  FLEXIO_SPI_MasterInit(&spiDev, &masterConfig,
                        MASTER_FLEXIO_SPI_CLOCK_FREQUENCY);
  ...
  /* Set up slave first */
  LPSPI_SlaveTransferCreateHandle(SLAVE_LPSPI_BASEADDR, &g_s_handle,
                                  LPSPI_SlaveUserCallback, NULL);
  ...
  DMAMUX_SetSource(EXAMPLE_FLEXIO_SPI_DMAMUX_BASEADDR,
                   FLEXIO_SPI_TX_DMA_LPSPI_CHANNEL, EXAMPLE_TX_DMA_SOURCE);
  DMAMUX_SetSource(EXAMPLE_FLEXIO_SPI_DMAMUX_BASEADDR,
                   FLEXIO_SPI_RX_DMA_LPSPI_CHANNEL, EXAMPLE_RX_DMA_SOURCE);
  DMAMUX_EnableChannel(EXAMPLE_FLEXIO_SPI_DMAMUX_BASEADDR,
                       FLEXIO_SPI_TX_DMA_LPSPI_CHANNEL);
  DMAMUX_EnableChannel(EXAMPLE_FLEXIO_SPI_DMAMUX_BASEADDR,
                       FLEXIO_SPI_RX_DMA_LPSPI_CHANNEL);
  EDMA_CreateHandle(&txHandle, EXAMPLE_FLEXIO_SPI_DMA_LPSPI_BASEADDR,
                    FLEXIO_SPI_TX_DMA_LPSPI_CHANNEL);
  EDMA_CreateHandle(&rxHandle, EXAMPLE_FLEXIO_SPI_DMA_LPSPI_BASEADDR,
                    FLEXIO_SPI_RX_DMA_LPSPI_CHANNEL);
